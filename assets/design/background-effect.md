...
### 流体手感（`FLUID` 对象）
静止 = 轻微浮动的清晰形状；移动 = 注入速度 → 墨水平流成流动拖尾；停下 = 拖尾消散、形状收拢但持续呼吸。
| 参数 | 当前值 | 说明 |
|------|--------|------|
...
| `velStop` | `0` | 速度低于此值直接归零。**保持 `0`**：交给 `velDecay` 阻尼即可让静止变清晰，避免和 `idleAmp` 互相打架（`idleAmp` 一旦小于 `velStop` 会被直接吃掉、毫无效果）|
| `force` | `0.6` | 鼠标速度→注入速度，**越大流动越狂野** |
| `flow` | `0.7` | 平流强度（墨水每单位速度移动几格）|
| `iter` | `4` | 解算迭代次数，越大swirl越平滑越耗性能 |
| `idleAmp` | `0.07` | 静止时持续注入的微弱"浮动力"，**让猫猫不会完全冻死**。调大会让浮动更明显，但要配合上面 `velDecay` 一起调，避免稳态幅度过大把形状糊掉 |
| `idleFreq` | `0.0015` | 浮动力的摆动速度，越大浮动越快/越明显 |
